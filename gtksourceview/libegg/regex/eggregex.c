...
match_error (gint errcode)
{
  ...
  if (errcode < 0 || errcode >= G_N_ELEMENTS(match_errors))
    errcode = 0;
...
static GList *
split_replacement (EggRegex       *regex,
		   const gchar  *replacement,
		   GError      **error)
{
  GList *list = NULL;
  ...
  while (*p)
      ...
      if (*p == '\\')
	{
	  ...
	  start = p = expand_escape (regex, replacement, p, data, error);
	  if (p == NULL)
	    {
			   ...
			   GError           **error)
{
  ...
  list = split_replacement (regex, string_to_expand, &tmp_error);
  if (tmp_error != NULL)
    {
      g_propagate_error (error, tmp_error);
      return NULL;
		 ...
		 GError           **error)
{
  ...
  list = split_replacement (regex, replacement, &tmp_error);
  if (tmp_error != NULL)
    {
      g_propagate_error (error, tmp_error);
      return NULL;
